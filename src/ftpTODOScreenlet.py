...
import pango
import screenlets
from screenlets.options import ColorOption, IntOption, StringOption, AccountOption
import sys
import time
...
class ftpTODOScreenlet(screenlets.Screenlet):
    """This Screenlet shows a simple todo list. A file on a ftp server is used to store the list."""

    __name__    = 'ftpTODOScreenlet'
    __version__ = 'Beta'
    ...
    def __init__ (self, **keyword_args):
        ...
        self.add_options_group("FTP", "FTP connection settings")
        
        opt_hostname = StringOption("FTP", "hostname", self.hostname, "Hostname (without ftp://)", "Hostname")
        self.add_option(opt_hostname)
        
        opt_hostdir = StringOption("FTP", "hostdir", self.hostdir, "Directory", "The directory on the server")
        self.add_option(opt_hostdir)
        
        opt_login = AccountOption("FTP", "login", self.login, "Username and password", "Username and password")
        self.add_option(opt_login)
    
    def on_after_set_atribute(self,name, value):
        if name == "login" and self._loaded:
            self._backend = backend_ftp.FTPTaskBackend(os.path.expanduser("~/.temptasks.xml"), self.hostname, self.hostdir, self.login[0], self.login[1], self._cb_backend_tasks_loaded, self._cb_backend_task_added, self._cb_backend_task_removed, self._cb_backend_task_updated, self._cb_backend_task_error)
            self._load_tasks()
...
if __name__ == '__main__':
    ...
    screenlets.session.create_session(ftpTODOScreenlet)
